...
LinuxTargetHelper.prototype.computeMimeTypeFiles = async function (...args) {
    ...
    const xmlPath = pathUtil.join(__dirname, '../linux-files/com.crispstrobe.brickwright.mime.xml');
    const rawXml = await fsPromises.readFile(xmlPath, 'utf-8');
    const newXml = rawXml.replace(/com\.crispstrobe\.brickwright/g, packageJSON.name);
